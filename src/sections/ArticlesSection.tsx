...
import { articlesData } from '../data/newsBlogsData';
...
const ArticlesSection: React.FC<ArticlesSectionProps> = ({ currentPage, onPageChange, onReadArticle }) => {
            ...
            <div className="flex space-x-6 transition-transform duration-500 ease-in-out">
              {currentArticles.map((article) => (
                <div key={article.id} className="flex-shrink-0 w-80">
                  <ArticleCard article={article} onReadArticle={onReadArticle} />
                </div>
              ))}
            </div>
          </div>
